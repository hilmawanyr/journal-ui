<?php
	...
	$root = $dom->createElement('ArticlesList');
	...
	foreach ($collection as $key => $value) {
		...
		if ($value['title']) {
			$title_node = $dom->createElement('Title', $value['title']);
		...
		if ($value['url']) {	
			$url_node = $dom->createElement('URL', htmlspecialchars($value['url']));
		...
		if ($value['license']) {
			...
			$article_node->appendChild($license_node);
		}
